...
const Personnel = () => {
  ...
  const fetchFromAPI = async (isPolling = false) => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/Personnel/collaborateurs-by-service/${userId}`)
        ...
        setPersonnelData((prev) => {
          ...
          const mergedCollaborators = [
            ...prev.collaborators.filter(
              (item) => !normalizedData.collaborators.some((newItem) => newItem.id === item.id),
            ),
  ...
  const fetchPersonnel = async (forceRefresh = false, isPolling = false) => {
    try {
      ...
      if (await fetchFromAPI(isPolling)) {
        // Only show success toast for manual refresh or initial API load (not polling)
        if (!isPolling) {
          const message = forceRefresh ? "Personnel data refreshed successfully" : "Personnel data loaded successfully"
      ...
      if (!isPolling) {
        toast.error(`Error loading personnel data: ${error.message}`, { autoClose: 5000 })
      }
    } finally {
      if (!isPolling) {
        setLoading(false)
              ...
              <div className="card-content">
                {loading ? (
                  ...
                  <div className="no-data-message">
                    ...
                    <p>
                      {personnelData.collaborators.length === 0 ? "No personnel found" : "No matching personnel found"}
                    </p>
